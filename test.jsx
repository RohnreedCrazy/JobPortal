...
const headCells = [
  ...
  { id: 'applicationDate', numeric: false, disablePadding: false, label: 'Date of Application' },
...
const ApplicationTableList = () => {
  ...
  const handleSearch = (event) => {
    const filteredRows = applications.filter((application) => {
      return application.jobTitle?.toLowerCase().includes(event.target.value.toLowerCase());
    });
              ...
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.jobTitle)}
                      role="checkbox"
                      aria-checked={isItemSelected}
                      tabIndex={-1}
                      key={row._id || `${row.jobTitle}-${index}`}
